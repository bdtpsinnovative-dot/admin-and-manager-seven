export type LocationInput = {
  countryCode?: string | null
  country?: string | null
  region?: string | null
  city?: string | null
}

const thaiProvinceCodes: Record<string, string> = {
  "10": "กรุงเทพมหานคร", "11": "สมุทรปราการ", "12": "นนทบุรี", "13": "ปทุมธานี", "14": "พระนครศรีอยุธยา",
  "15": "อ่างทอง", "16": "ลพบุรี", "17": "สิงห์บุรี", "18": "ชัยนาท", "19": "สระบุรี", "20": "ชลบุรี",
  "21": "ระยอง", "22": "จันทบุรี", "23": "ตราด", "24": "ฉะเชิงเทรา", "25": "ปราจีนบุรี", "26": "นครนายก",
  "27": "สระแก้ว", "30": "นครราชสีมา", "31": "บุรีรัมย์", "32": "สุรินทร์", "33": "ศรีสะเกษ", "34": "อุบลราชธานี",
  "35": "ยโสธร", "36": "ชัยภูมิ", "37": "อำนาจเจริญ", "38": "บึงกาฬ", "39": "หนองบัวลำภู", "40": "ขอนแก่น",
  "41": "อุดรธานี", "42": "เลย", "43": "หนองคาย", "44": "มหาสารคาม", "45": "ร้อยเอ็ด", "46": "กาฬสินธุ์",
  "47": "สกลนคร", "48": "นครพนม", "49": "มุกดาหาร", "50": "เชียงใหม่", "51": "ลำพูน", "52": "ลำปาง",
  "53": "อุตรดิตถ์", "54": "แพร่", "55": "น่าน", "56": "พะเยา", "57": "เชียงราย", "58": "แม่ฮ่องสอน",
  "60": "นครสวรรค์", "61": "อุทัยธานี", "62": "กำแพงเพชร", "63": "ตาก", "64": "สุโขทัย", "65": "พิษณุโลก",
  "66": "พิจิตร", "67": "เพชรบูรณ์", "70": "ราชบุรี", "71": "กาญจนบุรี", "72": "สุพรรณบุรี", "73": "นครปฐม",
  "74": "สมุทรสาคร", "75": "สมุทรสงคราม", "76": "เพชรบุรี", "77": "ประจวบคีรีขันธ์", "80": "นครศรีธรรมราช",
  "81": "กระบี่", "82": "พังงา", "83": "ภูเก็ต", "84": "สุราษฎร์ธานี", "85": "ระนอง", "86": "ชุมพร",
  "90": "สงขลา", "91": "สตูล", "92": "ตรัง", "93": "พัทลุง", "94": "ปัตตานี", "95": "ยะลา", "96": "นราธิวาส",
}

export function normalizeCountryCode(value: unknown) {
  const normalized = typeof value === "string" ? value.trim().toUpperCase() : ""
  if (/^[A-Z]{2}$/.test(normalized) && normalized !== "XX") return normalized
  return ({ THAILAND: "TH", "ประเทศไทย": "TH", "ไทย": "TH" } as Record<string, string>)[normalized] || null
}

export function countryLabel(value: unknown) {
  const code = normalizeCountryCode(value)
  if (!code) return typeof value === "string" ? value.trim() : ""
  try {
    return new Intl.DisplayNames(["th"], { type: "region" }).of(code) || code
  } catch {
    return code
  }
}

function cleanPart(value: string | null | undefined) {
  const cleaned = value?.trim().replace(/\s+/g, " ")
  return cleaned || null
}

function normalizeThaiRegion(value: string | null) {
  if (!value) return null
  const compact = value.toLowerCase().replace(/[\s._\-_/()]/g, "")
  if (thaiProvinceCodes[value]) return thaiProvinceCodes[value]
  if (compact === "กรุงเทพ" || compact === "กรุงเทพมหานคร" || compact === "bangkok") return "กรุงเทพมหานคร"
  return value.replace(/^changwat\s+/i, "").trim() || null
}

function normalizeThaiCity(value: string | null, region: string | null) {
  if (!value) return null
  const normalized = value.replace(/^khet\s+/i, "").replace(/^amphoe\s+/i, "").trim()
  const compact = normalized.toLowerCase().replace(/[\s._\-_/()]/g, "")
  if (region === "กรุงเทพมหานคร" && ["bangkok", "กรุงเทพ", "กรุงเทพมหานคร"].includes(compact)) return null
  return normalized || null
}

export function normalizeLocation(input: LocationInput) {
  const code = normalizeCountryCode(input.countryCode) || normalizeCountryCode(input.country)
  const country = countryLabel(code || input.country)
  const region = code === "TH" ? normalizeThaiRegion(cleanPart(input.region)) : cleanPart(input.region)
  const city = code === "TH" ? normalizeThaiCity(cleanPart(input.city), region) : cleanPart(input.city)
  const parts = [country, region, city].filter(Boolean)
  return {
    countryCode: code,
    country: country || null,
    region,
    city,
    label: parts.join(" / ") || null,
  }
}

export function isMainAnalyticsEvent(event: { is_countable?: boolean | null; traffic_type?: string | null }) {
  return event.is_countable !== false && event.traffic_type !== "bot" && event.traffic_type !== "internal"
}

export function displayTrafficType(value: string | null | undefined) {
  if (value === "internal") return "ภายในบริษัท"
  if (value === "bot") return "บอท"
  if (value === "customer" || value === "unknown") return "ผู้เข้าชมที่นับได้"
  return value || "ไม่ระบุ"
}

export function sanitizeCategoryName(name?: string | null): string {
  if (!name) return "ไม่ระบุหมวดหมู่"
  const clean = name.split("(")[0]?.trim()
  return clean || name.trim() || "ไม่ระบุหมวดหมู่"
}


