...
export default function DispatchMonitorPage() {
  ...
  async function loadData() {
    ...
    if (res.success) {
      ...
      alert("โหลดข้อมูลไม่สำเร็จ: " + res.error)
    }
    setLoading(false)
  }
  ...
  const handleApproveStock = async (orderId: number, orderCode: string, items: any[]) => {
    if (!confirm(`ยืนยันการรับชำระเงินและ "ตัดสต็อก" บิล ${orderCode} ใช่หรือไม่? \n(สินค้าจะถูกหักออกจากคลังทันที)`)) return
    
    setApprovingId(orderId)
    const res = await approveAndCutStock(orderId, orderCode, items)
    
    if (res.success) {
      alert("อนุมัติรับชำระเงิน และ หักสต็อกออกจากคลังเรียบร้อยแล้ว!")
      await loadData()
    } else {
  ...
  const handleCompleteOrder = async (orderId: number, orderCode: string, items: any[]) => {
    if (!confirm(`ยืนยันการจัดส่ง/ส่งมอบสินค้า ในบิล ${orderCode} เรียบร้อยแล้วใช่หรือไม่?`)) return
