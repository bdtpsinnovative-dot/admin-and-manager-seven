...
export default function PropBoxCalculator({ products, filteredProducts, selectedProducts }: PropBoxCalculatorProps) {
  ...
  return (
    <div className="mb-4 bg-white border border-slate-200 rounded p-4 text-slate-800">
      <div className="flex flex-col lg:flex-row lg:items-start gap-4 mb-4">
        <div className="flex-1">
          <p className="text-sm font-bold text-slate-900">คำนวณกล่องมาตรฐาน Props</p>
          <p className="text-xs text-slate-500 mt-0.5">
            เทียบขนาดสินค้า L x W x T กับกล่องมาตรฐาน และเลือกกล่องเล็กสุดที่ใส่ได้
          </p>
        </div>

        <div className="grid grid-cols-3 gap-2 w-full lg:w-auto text-center">
          <div className="rounded bg-slate-50 px-3 py-1.5 border border-slate-200">
            <p className="text-[10px] font-bold uppercase text-slate-400">ทั้งหมด</p>
            <p className="text-lg font-black text-slate-900 leading-tight">{totalBoxSummary.total}</p>
          </div>
          <div className="rounded bg-blue-50/50 px-3 py-1.5 border border-blue-200">
            <p className="text-[10px] font-bold uppercase text-blue-400">ที่แสดง</p>
            <p className="text-lg font-black text-blue-700 leading-tight">{filteredBoxSummary.total}</p>
          </div>
          <div className="rounded bg-green-50/50 px-3 py-1.5 border border-green-200">
            <p className="text-[10px] font-bold uppercase text-green-500">ที่เลือก</p>
            <p className="text-lg font-black text-green-700 leading-tight">{selectedBoxSummary.total}</p>
          </div>
        </div>
      </div>

      <div className="mb-2 flex items-center justify-between gap-3 border-t border-slate-100 pt-3">
        <p className="text-xs font-bold text-slate-500">สรุปกล่องสำหรับ {activeBoxScope}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
        {PROP_BOXES.map(box => (
          <div key={box.id} className="rounded border border-slate-200 bg-slate-50 px-3 py-2">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className={`text-xs font-black ${getToneClass(box.tone)}`}>{box.label}</p>
                <p className="text-[10px] text-slate-400 whitespace-nowrap">{box.size} cm</p>
              </div>
              <p className="text-xl font-black text-slate-900 leading-none">{activeBoxSummary.counts[box.id]}</p>
            </div>
          </div>
        ))}
      </div>
