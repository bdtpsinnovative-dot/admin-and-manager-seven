...
function SummaryStrip({ data, tab }: { data: AudienceAnalytics["summary"]; tab: "products" | "personas" }) {
  if (tab === "products") {
    ...
    return <section className="mb-5 rounded-2xl border border-[var(--algorithm-rule)] bg-[var(--algorithm-surface-soft)] p-4 sm:p-5">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--algorithm-accent-strong)]">อ่านเร็วจากตารางสินค้า</p><h2 className="mt-1 font-[var(--font-display)] text-2xl font-semibold tracking-[-0.04em]">ผู้ชมสินค้าโดยรวมสนใจอะไร</h2></div><p className="text-[10px] leading-4 text-[var(--algorithm-muted)]">สัดส่วนอุปกรณ์และช่องทางคิดจากการดูสินค้าไม่ซ้ำ · ตัด Bot / Internal แล้ว</p></div>
      <p className="mt-3 text-sm leading-6 text-[var(--algorithm-ink)]">สรุปเด่น: ผู้ชมใช้ <strong>{summary.device.value || "ยังไม่ระบุอุปกรณ์"}</strong> มากที่สุด · ใช้ <strong>{summary.browser.value || "ยังไม่ระบุเบราว์เซอร์"}</strong> · พบช่องทาง <strong>{summary.source.value || "ยังไม่ระบุช่องทาง"}</strong> มากที่สุด</p>
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <SummaryMetric label="ช่องทางที่พบมากสุด" metric={summary.source} />
        <SummaryMetric label="สถานที่ที่พบมากสุด" metric={summary.location} />
        <SummaryMetric label="หมวดที่ถูกดูมากสุด" metric={summary.category} />
  ...
  return <section className="mb-5 rounded-2xl border border-[var(--algorithm-rule)] bg-[var(--algorithm-surface-soft)] p-4 sm:p-5">
    <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--algorithm-accent-strong)]">อ่านเร็วจากตารางผู้ชม</p><h2 className="mt-1 font-[var(--font-display)] text-2xl font-semibold tracking-[-0.04em]">ผู้ชมส่วนใหญ่ใช้งานอย่างไร</h2></div><p className="text-[10px] leading-4 text-[var(--algorithm-muted)]">นับหนึ่งโปรไฟล์ต่อหนึ่งคน · Anonymous หมายถึง browser profile ไม่ใช่บุคคลที่ยืนยันแล้ว</p></div>
    <p className="mt-3 text-sm leading-6 text-[var(--algorithm-ink)]">สรุปเด่น: ผู้ชมส่วนใหญ่ใช้ <strong>{summary.device.value || "ยังไม่ระบุอุปกรณ์"}</strong> · ระบบ <strong>{summary.os.value || "ยังไม่ระบุระบบ"}</strong> · เบราว์เซอร์ <strong>{summary.browser.value || "ยังไม่ระบุเบราว์เซอร์"}</strong> และแหล่งที่มาครั้งแรกคือ <strong>{summary.source.value || "ยังไม่ระบุช่องทาง"}</strong></p>
    <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      <SummaryMetric label="อุปกรณ์ส่วนใหญ่" metric={summary.device} />
      <SummaryMetric label="ระบบปฏิบัติการส่วนใหญ่" metric={summary.os} />
      <SummaryMetric label="เบราว์เซอร์ส่วนใหญ่" metric={summary.browser} />
      <SummaryMetric label="แหล่งที่มาครั้งแรก" metric={summary.source} />
      <SummaryMetric label="สถานที่ที่พบมากสุด" metric={summary.location} />
      <SummaryMetric label="Persona ที่พบมากสุด" metric={summary.persona} />
    </div>
    <div className="mt-3 flex flex-wrap gap-x-6 gap-y-2 border-t border-[var(--algorithm-rule)] pt-3 text-xs text-[var(--algorithm-muted)]"><span>ผู้ชม <strong className="font-mono text-[var(--algorithm-ink)]">{number(summary.viewers)}</strong> โปรไฟล์</span><span>Sessions <strong className="font-mono text-[var(--algorithm-ink)]">{number(summary.sessions)}</strong></span><span>หน้าเข้าชม <strong className="font-mono text-[var(--algorithm-ink)]">{number(summary.pageViews)}</strong></span><span>เวลาเฉลี่ยต่อครั้ง <strong className="text-[var(--algorithm-ink)]">{seconds(summary.averageSessionSeconds)}</strong></span></div>
  </section>
}
...
export default function AudienceAnalyticsClient({ data }: { data: AudienceAnalytics }) {
  ...
  return <div className="algorithm-shell relative min-h-screen overflow-x-clip bg-[var(--algorithm-paper)] font-[var(--font-body)] text-[var(--algorithm-ink)]"><main className="relative mx-auto max-w-[1800px] px-3 py-3 sm:px-5 sm:py-5 lg:px-8 lg:py-8"><div className="rounded-[2rem] border border-[var(--algorithm-rule)] bg-[var(--algorithm-surface)] p-3 shadow-[var(--algorithm-shadow)] sm:p-5 lg:p-7"><header className="flex flex-col gap-4 border-b border-[var(--algorithm-rule)] pb-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><Link href="/algorithm" className="inline-flex min-h-10 items-center gap-2 rounded-full bg-[var(--algorithm-surface-soft)] px-4 text-xs font-bold hover:bg-[var(--algorithm-accent-soft)]"><ArrowLeft className="h-4 w-4" />กลับภาพรวม</Link><div><p className="font-[var(--font-display)] text-lg font-semibold">Audience Analytics</p><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--algorithm-muted)]">Product · Persona · Journey</p></div></div><div className="flex items-center gap-1 rounded-full border border-[var(--algorithm-rule)] p-1">{ranges.map((range) => <Link key={range.days} href={`/algorithm/audience?range=${range.days}`} className={`rounded-full px-3 py-2 text-xs font-bold ${data.rangeDays === range.days ? "bg-[var(--algorithm-ink)] text-white" : "text-[var(--algorithm-muted)] hover:bg-[var(--algorithm-accent-soft)]"}`}>{range.label}</Link>)}</div></header><section className="flex flex-col justify-between gap-6 py-7 lg:flex-row lg:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--algorithm-accent-soft)] px-3 py-1.5 text-[10px] font-bold text-[var(--algorithm-accent-strong)]"><Eye className="h-3.5 w-3.5" />ข้อมูลการใช้งานจริง</div><h1 className="font-[var(--font-display)] text-4xl font-semibold tracking-[-0.06em] sm:text-5xl">ดูเส้นทางของผู้ชม</h1><p className="mt-4 max-w-3xl text-sm leading-6 text-[var(--algorithm-muted)]">{data.productStartedAt} · ช่วงเวลาที่เลือก {data.rangeDays} วัน · ยอดหลักตัด Bot/Internal ออก</p></div><div className="grid grid-cols-2 gap-3 text-right"><div className="rounded-2xl bg-[var(--algorithm-surface-soft)] px-5 py-3"><p className="font-mono text-2xl font-bold">{number(data.products.length)}</p><p className="text-[10px] text-[var(--algorithm-muted)]">สินค้า Prop</p></div><div className="rounded-2xl bg-[var(--algorithm-surface-soft)] px-5 py-3"><p className="font-mono text-2xl font-bold">{number(data.personas.length)}</p><p className="text-[10px] text-[var(--algorithm-muted)]">โปรไฟล์ผู้ชม</p></div></div></section>{data.error && <div className="mb-5 rounded-2xl border border-[var(--algorithm-danger)]/30 bg-[var(--algorithm-hot-soft)] p-4 text-sm text-[var(--algorithm-danger)]">{data.error}</div>}<div className="mb-5 flex items-center gap-2 rounded-full bg-[var(--algorithm-surface-soft)] p-1"><button onClick={() => setTab("products")} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold ${tab === "products" ? "bg-[var(--algorithm-ink)] text-white" : "text-[var(--algorithm-muted)]"}`}><Eye className="h-3.5 w-3.5" />วิเคราะห์สินค้า</button><button onClick={() => setTab("personas")} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold ${tab === "personas" ? "bg-[var(--algorithm-ink)] text-white" : "text-[var(--algorithm-muted)]"}`}><UsersRound className="h-3.5 w-3.5" />วิเคราะห์ Persona</button><span className="ml-auto hidden items-center gap-1 text-[10px] text-[var(--algorithm-muted)] sm:flex"><RefreshCw className="h-3 w-3" />อ่านจาก Raw Event</span></div><div className="mb-5 flex flex-col gap-3 rounded-2xl border border-[var(--algorithm-rule)] bg-[var(--algorithm-surface-soft)] p-4">{tab === "products" ? <><div className="flex flex-wrap items-end gap-3"><label className="flex min-w-[240px] flex-1 flex-col gap-1 text-[10px] font-bold text-[var(--algorithm-muted)]"><span>ค้นหาสินค้า</span><input value={productQuery} onChange={(event) => setProductQuery(event.target.value)} placeholder="ชื่อสินค้า, SKU, หมวด, source..." className="h-10 rounded-xl border border-[var(--algorithm-rule)] bg-white px-3 text-xs outline-none focus:border-[var(--algorithm-blue)]" /></label><Select label="หมวดสินค้า" value={productCategory} onChange={setProductCategory} options={data.products.map((item) => item.category)} /><Select label="อุปกรณ์" value={productDevice} onChange={setProductDevice} options={data.products.map((item) => item.primaryDevice || "")} /><Select label="เบราว์เซอร์" value={productBrowser} onChange={setProductBrowser} options={data.products.map((item) => item.primaryBrowser || "")} /><Select label="สี" value={productColor} onChange={setProductColor} options={data.products.map((item) => item.color || "")} /><Select label="สถานที่" value={productLocation} onChange={setProductLocation} options={productLocationOptions} /><Select label="ช่องทาง" value={productSource} onChange={setProductSource} options={productSourceOptions} /><SortSelect value={productSortKey} onChange={setProductSortKey} options={productSortOptions} /><DirectionSelect value={productSortDirection} onChange={setProductSortDirection} /></div><div className="flex flex-wrap items-center gap-2 text-[10px] text-[var(--algorithm-muted)]"><Filter className="h-3.5 w-3.5" />ตัวกรองนี้ใช้กับตารางสินค้าเท่านั้น · กดหัวคอลัมน์เพื่อเรียง มาก→น้อย หรือ น้อย→มาก</div></> : <><div className="flex flex-wrap items-end gap-3"><label className="flex min-w-[240px] flex-1 flex-col gap-1 text-[10px] font-bold text-[var(--algorithm-muted)]"><span>ค้นหาผู้ชม</span><input value={personaQuery} onChange={(event) => setPersonaQuery(event.target.value)} placeholder="ID, ชื่อ, สถานที่, Persona..." className="h-10 rounded-xl border border-[var(--algorithm-rule)] bg-white px-3 text-xs outline-none focus:border-[var(--algorithm-blue)]" /></label><Select label="หมวดที่สนใจ" value={personaCategory} onChange={setPersonaCategory} options={personaCategoryOptions} /><Select label="กลุ่มพฤติกรรม" value={personaBehavior} onChange={setPersonaBehavior} options={labelOptions} /><Select label="ช่องทาง" value={personaSource} onChange={setPersonaSource} options={personaSourceOptions} /><Select label="สถานที่" value={personaLocation} onChange={setPersonaLocation} options={personaLocationOptions} /><Select label="อุปกรณ์" value={personaDevice} onChange={setPersonaDevice} options={data.personas.map((item) => item.device || "")} /><Select label="ระบบปฏิบัติการ" value={personaOs} onChange={setPersonaOs} options={data.personas.map((item) => item.os || "")} /><Select label="เบราว์เซอร์" value={personaBrowser} onChange={setPersonaBrowser} options={data.personas.map((item) => item.browser || "")} /><label className="flex min-w-[130px] flex-col gap-1 text-[10px] font-bold text-[var(--algorithm-muted)]"><span>ขั้นต่ำจำนวนครั้งที่เข้าเว็บ</span><input type="number" min="0" value={minSessions} onChange={(event) => setMinSessions(event.target.value)} className="h-10 rounded-xl border border-[var(--algorithm-rule)] bg-white px-3 text-xs outline-none focus:border-[var(--algorithm-blue)]" /></label><SortSelect value={personaSortKey} onChange={setPersonaSortKey} options={personaSortOptions} /><DirectionSelect value={personaSortDirection} onChange={setPersonaSortDirection} /></div><div className="flex flex-wrap items-center gap-2 text-[10px] text-[var(--algorithm-muted)]"><Filter className="h-3.5 w-3.5" />ตัวกรองนี้ใช้กับตารางผู้ชมเท่านั้น · กดหัวคอลัมน์เพื่อเรียง มาก→น้อย หรือ น้อย→มาก</div></>}</div><SummaryStrip data={data.summary} tab={tab} />{tab === "products" ? <ProductTable products={filteredProducts} query={productQuery} sortKey={productSortKey} direction={productSortDirection} onSort={changeProductSort} /> : <PersonaTable personas={filteredPersonas} query={personaQuery} categoryFilter={personaCategory} behaviorFilter={personaBehavior} sourceFilter={personaSource} locationFilter={personaLocation} deviceFilter={personaDevice} osFilter={personaOs} browserFilter={personaBrowser} sortKey={personaSortKey} direction={personaSortDirection} onSort={changePersonaSort} />}</div></main></div>
}
